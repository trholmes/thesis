...
The sensors are n-type silicon wafers with a voltage applied, and a passing charged particle produces thousands of electron-hole pairs inside the material, which drift in the electric field towards the mounted read-out system. A hit occurs when the resulting current becomes large enough to pass a threshold designed to suppress noise. \autoref{fig:pix_thresh} shows the noise distribution and threshold value in the pixels. The threshold is tuned per pixel such that its value is approximately 25 times the average noise for that pixel.  

\begin{centering}
\begin{figure}[bth]
\myfloatalign
\includegraphics[width=.90\linewidth]{figures/atlas/pix_noise_all.eps}
\includegraphics[width=.90\linewidth]{figures/atlas/pix_thresh_all.eps}
\caption{Plots of noise and thresholds in the Pixel Detector with an entry for each pixel. Top is the distribution of charge in noise hits in the detector. Bottom is the threshold value, which is tuned for each pixel \cite{pixel}.}
\label{fig:pix_thresh}
\end{figure}
\end{centering}
...
The solenoid is inside the calorimeter volume and provides a uniform 2 T magnetic field for particles traveling through the \ac{ID}. This axial field causes the trajectories of charged particles to bend in the $x-y$ plane, and measurements of the curvature of these trajectories give the most accurate \pt measurement for many particles according to the equation
